      ...
      for (let i = 0; i < numCuts; i++) {
        ...
        try {
          ...
          if (process.env.NODE_ENV === 'development') {
            ...
            const requestData = {
              model: 'dall-e-3',
              prompt: `${cutImagePrompt}. ${shotSize} 샷, ${angleDirection} 앵글, ${sceneInfo.lighting} 조명, 시네마틱한 구도, 고품질 이미지`,
              n: 1,
              size: '1024x1024',
              quality: 'standard',
              style: 'natural'
            ...
            console.log(`✅ 컷 이미지 생성 완료 (${i + 1}/${parsedCuts.length}):`, { 
              title: cut.title,
              imageUrl: imageUrl.substring(0, 50) + '...' 
            })
            
            // 이미지 URL을 컷 데이터에 추가
            cut.imageUrl = cutImageUrl
            
          } catch (imageError) {
            console.error(`❌ 컷 이미지 생성 실패 (${i + 1}/${parsedCuts.length}):`, { 
              title: cut.title, 
            ...
            // 이미지 생성 실패 시 빈 문자열로 설정
            cut.imageUrl = ''
          }
        } catch (imageError) {
          console.error(`❌ 컷 이미지 생성 실패:`, imageError.message)
...
app.post('/api/cuts/generate', async (req, res) => {
  try {
    ...
    console.log('🎬 컷 이미지 생성 시작:', { totalCuts: cuts.length })
    
    // 모든 컷에 대해 이미지 생성 후 저장
    for (let i = 0; i < cuts.length; i++) {
      const cut = cuts[i]
      try {
        console.log(`🎬 컷 이미지 생성 중 (${i + 1}/${cuts.length}):`, { title: cut.title, description: cut.description })
        
        // 컷 이미지 생성
        const imageRequestData = {
          cutDescription: cut.description || cut.title,
          shotSize: cut.shootingPlan?.shotSize || 'MS',
          angleDirection: cut.shootingPlan?.angleDirection || 'Eye-Level',
          lightingSetup: {
            mainLight: cut.shootingConditions?.lighting || '자연광'
          },
          style: 'cinematic',
          size: '1024x1024'
        }
        
        // OpenAI DALL-E 3 API 호출하여 이미지 생성
        const requestData = {
          model: 'dall-e-3',
          prompt: `${imageRequestData.cutDescription}. ${imageRequestData.shotSize} 샷, ${imageRequestData.angleDirection} 앵글, ${imageRequestData.lightingSetup.mainLight} 조명, ${imageRequestData.style} 스타일, 시네마틱한 구도, 고품질 이미지`,
          n: 1,
          size: imageRequestData.size,
          quality: 'standard',
          style: 'natural'
        }
        ...
        console.log(`✅ 컷 이미지 생성 완료 (${i + 1}/${cuts.length}):`, { 
          title: cut.title,
          imageUrl: imageUrl.substring(0, 50) + '...' 
        })
        
        // 이미지 URL을 컷 데이터에 추가
        cut.imageUrl = imageUrl
        
      } catch (imageError) {
        console.error(`❌ 컷 이미지 생성 실패 (${i + 1}/${cuts.length}):`, { 
          title: cut.title, 
          error: imageError.message,
          status: imageError.response?.status,
          statusText: imageError.response?.statusText,
    ...
    // 모든 컷과 이미지가 완성된 후 DB에 저장
    console.log('💾 모든 컷과 이미지 생성 완료, DB 저장 시작...')
    
    for (let i = 0; i < cuts.length; i++) {
      const cut = cuts[i]
      try {
        console.log(`💾 컷 DB 저장 중 (${i + 1}/${cuts.length}):`, { title: cut.title })
        ...
        console.log(`✅ 컷 저장 완료 (${i + 1}/${cuts.length}):`, { 
          cutId: savedCut._id, 
          scene: sceneData.scene, 
          imageUrl: cut.imageUrl ? '있음' : '없음' 
        })
        
      } catch (saveError) {
        console.error(`❌ 컷 저장 오류 (${i + 1}/${cuts.length}):`, saveError.message)
        // 저장 실패해도 계속 진행
        savedCuts.push(cut)
      }
    }
