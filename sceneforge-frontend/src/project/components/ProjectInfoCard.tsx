          ...
          <Grid item xs={12} {...({} as any)}>
            ...
            {isEditing ? (
              <TextField
                ...
                variant="outlined"
                size="small"
                placeholder="영화의 줄거리를 간단히 설명하세요"
              />
            ) : (
              <Typography variant="body1">{project.synopsis}</Typography>
            )}
          </Grid>
            ...
            <Grid container spacing={2}>
              <Grid item xs={6} md={3} {...({} as any)}>
                <Box sx={{ textAlign: 'center', p: 2, border: '1px solid', borderColor: 'divider', borderRadius: 1 }}>
                  ...
                  <Typography variant="h6">{project.participants?.length || 0}</Typography>
                  <Typography variant="caption" color="text.secondary">참여자</Typography>
                </Box>
              </Grid>
              <Grid item xs={6} md={3} {...({} as any)}>
                <Box sx={{ textAlign: 'center', p: 2, border: '1px solid', borderColor: 'divider', borderRadius: 1 }}>
                  <Movie sx={{ fontSize: 32, color: 'secondary.main', mb: 1 }} />
                  <Typography variant="h6">{project.scenes?.length || 0}</Typography>
                  <Typography variant="caption" color="text.secondary">장면</Typography>
                </Box>
              </Grid>
              <Grid item xs={6} md={3} {...({} as any)}>
                <Box sx={{ textAlign: 'center', p: 2, border: '1px solid', borderColor: 'divider', borderRadius: 1 }}>
                  <Tag sx={{ fontSize: 32, color: 'info.main', mb: 1 }} />
                  <Typography variant="h6">{project.tags?.length || 0}</Typography>
                  <Typography variant="caption" color="text.secondary">태그</Typography>
                </Box>
              </Grid>
              <Grid item xs={6} md={3} {...({} as any)}>
                <Box sx={{ textAlign: 'center', p: 2, border: '1px solid', borderColor: 'divider', borderRadius: 1 }}>
                  <AccessTime sx={{ fontSize: 32, color: 'warning.main', mb: 1 }} />
                  <Typography variant="h6">{project.genre?.length || 0}</Typography>
