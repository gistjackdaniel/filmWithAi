...
import ProjectCreationModal from '../../project/components/ProjectCreationModal';
    ...
    <Box sx={{ flexGrow: 1 }}>
      ...
      <Container maxWidth="lg" sx={{ mt: 4 }}>
        ...
        <Grid container spacing={2}>
          {projects.filter(project => project && project._id).length > 0 ? (
            // 프로젝트가 있는 경우: 프로젝트 카드들 표시
            projects.filter(project => project && project._id).map((project) => (
              <Grid item xs={12} sm={6} md={4} key={project._id} {...({} as any)}>
                <Card 
                  sx={{ 
                    cursor: 'pointer',
                    '&:hover': { transform: 'translateY(-2px)', transition: '0.2s' },
