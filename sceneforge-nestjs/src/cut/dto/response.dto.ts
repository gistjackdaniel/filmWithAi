...
export class CutDraftResponseDto {
  ...
  @ApiProperty({
    description: 'VFX 효과',
    example: '도시 배경 합성, 안개 효과',
  })
  vfxEffects: string;

  @ApiProperty({
    description: '음향 효과',
    example: '도시 배경음, 발걸음 소리',
  })
  soundEffects: string;

  @ApiProperty({
    description: '감독 노트',
    example: '자연스러운 걷기 연기, 주변을 둘러보는 시선 처리',
  })
  directorNotes: string;
  ...
  @ApiProperty({
    description: '내레이션',
    example: '그날, 그는 처음으로 이 도시에 발을 들였다',
  })
  narration: string;
  ...
  @ApiProperty({
    description: '제작 방법 선택 근거',
    example: '실사 촬영으로 자연스러운 도시 분위기 연출',
  })
  productionMethodReason: string;
  ...
  @ApiProperty({
    description: '특수 요구사항',
    type: SpecialRequirementsResponseDto,
  })
  specialRequirements: SpecialRequirementsResponseDto;
